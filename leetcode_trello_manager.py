...
def make_request(url, method, params=None, data=None, timeout=None, files=None):
    with requests.request(
        ...
        try:
            ...
            logging.error(
                f"Failed to decode JSON from response. URL: {url}, Response: {response.text}"
            )
            return None
...
def get_board_id(config, settings, name):
    boards = trello_request(
        config, settings, "me/boards", filter="open", entity="members"
    )
...
def delete_list(config, settings, board_id, list_name):
    list_id = next(
        lst["id"]
        for lst in trello_request(config, settings, f"{board_id}/lists")
...
def set_custom_board_background(config, settings, board_id, background_id):
    ...
    response = trello_request(
        config, settings, endpoint, method="PUT", entity="boards", value=background_id
    )
...
def attach_image_to_card(config, settings, card_id, topic):
    ...
    response = trello_request(
        config, settings, f"/cards/{card_id}/attachments", "POST", url=image_url
    )
    if not response:
...
def generate_all_due_dates(topics, start_date):
    return reduce(
        lambda acc, problems: acc + [get_next_working_day(acc[-1])] * len(problems),
        topics.values(),
        [get_next_working_day(start_date)],
    )[:-1]
...
def create_labels_for_board(config, settings, board_id):
    ...
    for label, color in label_colors.items():
        if label not in label_names:
            logging.info(
                f"Constructed resource string for label creation: {board_id}/labels"
            )
            trello_request(
                config,
                settings,
...
def process_retrospective_cards(config, settings, board_id, current_date):
    list_ids = {
        l["name"]: l["id"]
        for l in trello_request(config, settings, f"/boards/{board_id}/lists")
    }
...
def setup_board(config, settings, board_name, topics, current_date):
    ...
    if board_id is None:
        board_response = trello_request(
            config, settings, "", method="POST", name=board_name
        )
